...
namespace System {
    ...
    #ifdef PROJECT_ENABLE_ADC0
        ADC::ADC ADC::adc0 = {.adc = ADC0};
    #endif
    #ifdef PROJECT_ENABLE_ADC1
        ADC::ADC ADC::adc1 = {.adc = ADC1};
    #endif
...
void System::init() {
    ...
    delay_cycles(POWER_STARTUP_DELAY);
        ...
        delay_cycles(POWER_STARTUP_DELAY);
        {
            const DL_ADC12_ClockConfig clk_config = {
                 .clockSel      = DL_ADC12_CLOCK::DL_ADC12_CLOCK_SYSOSC,
                 .freqRange     = DL_ADC12_CLOCK_FREQ_RANGE::DL_ADC12_CLOCK_FREQ_RANGE_24_TO_32,
                 .divideRatio   = DL_ADC12_CLOCK_DIVIDE::DL_ADC12_CLOCK_DIVIDE_32,
            };
            static_assert(System::CLK::SYSOSC == 32e6);
            DL_ADC12_setClockConfig(ADC0, &clk_config);
        }
        ...
        delay_cycles(POWER_STARTUP_DELAY);
        {
            ...
            static_assert(System::CLK::SYSOSC == 32e6);
            DL_ADC12_setClockConfig(ADC1, &clk_config);
        }
...
//    DL_FlashCTL_executeClearStatus(); // ERRNO thing
    ...
    #ifdef PROJECT_ENABLE_UART0
    {
        ...
        uart0.partialInit();
        DL_GPIO_initPeripheralOutputFunction(IOMUX_PINCM21, IOMUX_PINCM21_PF_UART0_TX); // PA10   //PA10:TX, PA11:RX for LP
...
void System::ADC::ADC::_irq() {
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;

    vTaskNotifyGiveIndexedFromISR(_task, TASK_NOTIFICATION_ARRAY_INDEX_FOR_SYSTEM_ADC_IRQ, &xHigherPriorityTaskWoken);
    DL_ADC12_getPendingInterrupt(adc); // reset pending
...
uint16_t System::ADC::ChannelMap::getResult() const {
    return DL_ADC12_getMemResult(adc.adc, idx);
}
...
const uint32_t System::CANFD::getCANID(DL_MCAN_RxBufElement const * b) {
    if(b->xtd)  return b->id;
    else        return (b->id & 0x1FFC'0000) >> 18;
}

const uint32_t System::CANFD::getCANID(DL_MCAN_TxBufElement const * b) {
    if(b->xtd)  return b->id;
    else        return (b->id & 0x1FFC'0000) >> 18;
}
...
#endif
#ifdef PROJECT_ENABLE_ADC0
    extern "C" void ADC0_IRQHandler(void){ System::ADC::adc0._irq(); };
#endif
#ifdef PROJECT_ENABLE_ADC1
    extern "C" void ADC1_IRQHandler(void){ System::ADC::adc1._irq(); };
#endif
