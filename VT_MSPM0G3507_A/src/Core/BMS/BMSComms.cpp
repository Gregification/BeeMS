...
bool BMSComms::sendPacket(J1939_PF_e type, uint8_t J1939_JS, int8_t priorityOffset, void const * data, uint8_t len) {
    ...
    { // transmit
        ...
        DL_MCAN_getTxFIFOQueStatus(System::canFD0.reg, &tf);

        if(tf.freeLvl == 0) {
            System::canFD0.giveResource();
            return false;
        }

        DL_MCAN_writeMsgRam(System::canFD0.reg, DL_MCAN_MEM_TYPE_FIFO, tf.putIdx, &msg);
        DL_MCAN_TXBufAddReq(System::canFD0.reg, tf.getIdx);
